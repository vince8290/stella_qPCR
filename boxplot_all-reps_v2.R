...
# Create boxplots from transformed data frame
  ...
  for (cult in unique(df3$cultivar))  {
    ...
    for (og in unique(df$gene) ) {
        ...
        p <- ggplot(df3[(df3$cultivar==cult) & (df3$gene==og),], aes(x=reorder(gene.dpi, max_med), y=trans(expression))) +
          theme_bw() +
          theme(legend.position="none", axis.text = element_text(face="bold",size=14), 
                axis.text.x=element_text(angle=90, hjust=1),
                axis.title = element_text(face="bold",size=16)) +
          scale_y_continuous(breaks=breaks, labels=labels) +
          ggtitle(paste0(cult))+
          labs(x="days post inoculation", y="Expression") +
          geom_boxplot(outlier.color = "white", aes(fill=factor(dpi), alpha=0.1)) +
