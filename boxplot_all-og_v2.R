...
outputfolder = file.path(rawdatafolder, format(Sys.time(), "Results_all-og_%Y%m%d_%H%M%S"))  # outfolder: name of the folder to which graphs are exported
...
##############################################################################################
  ...
  df2 = df %>%
    ...
    summarise(med=median(expression), count=n()) %>%
...
# Create boxplots from transformed data frame
  ...
  for (cult in unique(df3$cultivar))  {
    ...
    p <- ggplot(df3[(df3$cultivar==cult),], aes(x=reorder(gene.dpi, max_med),y=trans(expression))) +
      theme_bw() +
      ...
      geom_hline(yintercept =trans(1), color="darkred") +
      geom_hline(yintercept =trans(0.5), color="red") +
    ...
    # reps
    for (rep in unique(df3$rep))  {
      # Create a vertical boxplot (several options available: violin etc. See ggplot plots in R supporting documentation)
      p <- ggplot(df3[(df3$cultivar==cult) & (df3$rep==rep),], aes(x=reorder(gene.dpi, max_med),y=trans(expression))) +
        theme_bw() +
        theme(legend.position="none",axis.text = element_text(face="bold",size=10), axis.text.x=element_text(angle=90, hjust=1),
